...
export default function AnalyzeCN({ mode = 'model' }: { mode?: 'actuals' | 'model' }) {
  ...
  // China dual-credit meter — the whole market's two-axis shape, market-level so
  // it stays constant while you drill. Additive; never touches the drill logic.
  const dc = useMemo(() => buildDualCredit(tree, scenario, pack.creditPrice ?? pack.fineRate), [tree, scenario, pack])
  const dcG = { clear: dc.oems.filter((o) => o.status === 'both-clear').length, self: dc.oems.filter((o) => o.status === 'self-offset').length, buy: dc.oems.filter((o) => o.creditsToBuy > 0.5).length }
  const dcBothOk = dc.totals.makers - dc.totals.makersOver
  // Credit position by drill scope (parent AND pool) — powers the credit
  // scoreboard so a maker/pool row shows its 积分 balance, not a "gap".
  const creditByScope = useMemo(() => {
    const m = new Map<string, { cafc: number; nev: number; cost: number; buy: number; short: boolean }>()
    for (const o of dc.oems) {
      // dedupe keys: when a maker's pool === its parent (e.g. the CN dataset, one
      // compliance entity per pool) the two keys collide and would double-count.
      // A genuine multi-parent pool still aggregates its members via the pool key.
      for (const key of new Set([o.parent, o.pool])) {
        const e = m.get(key) ?? { cafc: 0, nev: 0, cost: 0, buy: 0, short: false }
        e.cafc += o.cafcCredit; e.nev += o.nevBalance; e.cost += o.cost; e.buy += o.creditsToBuy; e.short = e.short || o.creditsToBuy > 0.5
        m.set(key, e)
      }
    }
    return m
  ...
  return (
    <div className="space-y-5">
      ...
      {/* THE VERDICT — restyled to the dual-credit bar (hero + China meter foot).
          Content/logic unchanged; only presentation. */}
      <div className="rise card relative overflow-hidden p-0">
        <span aria-hidden className="absolute inset-x-0 top-0 h-[3px]" style={{ background: node.status === 'exempt' ? '#D98005' : over ? '#E0484D' : '#0E9F6E' }} />
        <div className="flex flex-wrap items-start justify-between gap-x-8 gap-y-4 p-7">
        <div className="min-w-[280px] flex-1">
          <div className="label">Credit standing · 积分 · {scenario.year}</div>
          <p className="mt-3 max-w-2xl text-[21px] font-medium leading-[1.35] tracking-[-0.01em] text-ink-300">
            <b className="text-ink-100">{node.label}</b> banks{' '}
            <b className={dc.totals.cafcCredit >= 0 ? 'text-safe' : 'text-danger'}>{dc.totals.cafcCredit >= 0 ? '+' : '−'}{fmtInt(Math.abs(dc.totals.cafcCredit))}</b> fuel-economy credits and{' '}
            <b className={dc.totals.nevBalance >= 0 ? 'text-safe' : 'text-danger'}>{dc.totals.nevBalance >= 0 ? '+' : '−'}{fmtInt(Math.abs(dc.totals.nevBalance))}</b> EV-volume credits —{' '}
            <b className="text-ink-100">{dcBothOk} of {dc.totals.makers}</b> makers clear both. Compliance is a credit balance, not a line.
          </p>
          {dc.totals.makersOver > 0 && (
            <p className="mt-1.5 text-[12.5px] text-ink-400">
              {dc.totals.makersOver} makers must buy <b className="num text-ink-100">{fmtInt(dc.totals.creditsToBuy)}</b> credits ({fmtMoney(dc.totals.cost, pack.currency)}).{' '}
              {worstChild && (
                <>Biggest drag: <button onClick={() => worstChild.drillable && drillInto(worstChild.key)} className="font-semibold text-ink-100 underline decoration-dotted underline-offset-2 hover:text-brand">{worstChild.label}</button>.{' '}</>
              )}
              <button onClick={() => setScreen('creditbook')} className="font-semibold text-brand hover:underline">Open the credit book →</button>
            </p>
          )}
        </div>
        {/* trajectory — click a year to move the whole workspace there */}
        <div className="shrink-0">
          <div className="label mb-1.5">Fuel-economy trend by year</div>
        ...
        {/* China dual-credit compliance meter — the two-axis shape at a glance */}
        <div className="flex flex-wrap items-center gap-x-9 gap-y-4 border-t border-black/[0.05] px-7 py-5">
          <div className="shrink-0">
            <div className="dnum text-[26px] font-bold leading-none text-ink-100">{dcBothOk}<span className="text-[15px] font-semibold text-ink-500">/{dc.totals.makers}</span></div>
            <div className="label mt-1.5">clear both credit axes</div>
          </div>
          <div className="min-w-[240px] flex-1">
            <div className="flex h-2.5 gap-[3px]">
              {dcG.clear > 0 && <span className="rounded-full" style={{ flex: dcG.clear, background: '#0E9F6E' }} />}
              {dcG.self > 0 && <span className="rounded-full" style={{ flex: dcG.self, background: '#2E9E8F' }} />}
              {dcG.buy > 0 && <span className="rounded-full" style={{ flex: dcG.buy, background: '#E0484D' }} />}
            </div>
            <div className="mt-2.5 flex flex-wrap gap-x-5 gap-y-1 text-[11px] text-ink-400">
              <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-safe" /><b className="num font-semibold text-ink-200">{dcG.clear}</b> clear on both</span>
              <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full" style={{ background: '#2E9E8F' }} /><b className="num font-semibold text-ink-200">{dcG.self}</b> covered by own EV credits</span>
              <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-danger" /><b className="num font-semibold text-ink-200">{dcG.buy}</b> must buy credits</span>
            </div>
          </div>
          <button onClick={() => setScreen('creditbook')} className="btn-ghost shrink-0 px-3 py-1.5 text-[12px]"><Icon name="scale" size={13} /> Dual-credit ledger</button>
        </div>
      </div>

      {/* KPI strip — CREDITS (积分), not "the line". China compliance is a credit
          balance on two axes, never "over/under a limit". */}
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <div className={`card rise relative overflow-hidden p-4 ${dc.totals.cafcCredit >= 0 ? 'border-safe/25' : 'border-danger/25'}`}>
          <span className="absolute inset-x-0 top-0 h-[3px]" style={{ background: dc.totals.cafcCredit >= 0 ? '#0E9F6E' : '#E0484D' }} />
          <div className="label">Fuel-economy credits (CAFC)</div>
          <div className={`dnum mt-2 text-[27px] font-bold leading-none ${dc.totals.cafcCredit >= 0 ? 'text-safe' : 'text-danger'}`}>{dc.totals.cafcCredit >= 0 ? '+' : '−'}{fmtInt(Math.abs(dc.totals.cafcCredit))}</div>
          <div className="mt-2 text-[11px] text-ink-500">{dc.totals.cafcCredit >= 0 ? '正积分 · surplus' : '负积分 · deficit'}</div>
        </div>
        <div className={`card rise p-4 [animation-delay:60ms]`}>
          <div className="label">EV-volume credits (NEV)</div>
          <div className={`dnum mt-2 text-[27px] font-bold leading-none ${dc.totals.nevBalance >= 0 ? 'text-safe' : 'text-danger'}`}>{dc.totals.nevBalance >= 0 ? '+' : '−'}{fmtInt(Math.abs(dc.totals.nevBalance))}</div>
          <div className="mt-2 text-[11px] text-ink-500">ratio {Math.round(dc.ratio * 100)}% · earned {fmtInt(dc.totals.nevEarned)}</div>
        </div>
        <div className="card rise p-4 [animation-delay:120ms]">
          <div className="label">Credits to buy</div>
          <div className={`dnum mt-2 text-[27px] font-bold leading-none ${dc.totals.creditsToBuy > 0.5 ? 'text-danger' : 'text-safe'}`}>{fmtInt(dc.totals.creditsToBuy)}</div>
          <div className="mt-2 text-[11px] text-ink-500">{dc.totals.makersOver} of {dc.totals.makers} makers · {fmtMoney(dc.totals.cost, pack.currency)}</div>
        </div>
        <Stat className="rise [animation-delay:180ms]" label="Fleet fuel use (CAFC)" value={fmtNum(avgA, 1)} sub={`${pack.metricUnit} · fuel-economy axis`} />
        <Stat className="rise [animation-delay:240ms]" label="Registrations" value={fmtInt(regA)} sub={`${fmtInt(unitsA)} NEV-weighted`} />
      </div>

      {/* Bubble chart with drill — the CAFC (fuel-economy) axis. The mass-based
          target IS legitimate for China (GB 27999 per-vehicle target curve), but
          it is ONE of two axes; compliance is the credit balance, not this line. */}
      <Section className="rise [animation-delay:300ms]" title={`${sectionLabel} · fuel-economy (CAFC) axis`} right={
        ...
        <div className="relative">
          <LimitChart pack={pack} limitAt={limitAt} points={points} colorBy={colorByEff} height={360} onPick={drillInto} unitRef={unitRef} drag={dragCfg} logos={level <= 1}
            ghosts={ghostLines} corridor={corridor} stringency={stringency} view={chartView} draftLine={draftLine} limitLabel="CAFC TARGET · 达标值" />
        ...
        <p className="mt-3 rounded-lg border border-black/[0.05] bg-black/[0.015] px-3 py-2 text-[11px] leading-relaxed text-ink-500">
          <b className="text-ink-300">This line is the CAFC target (达标值), not a pass/fail limit.</b> It's the zero-credit boundary: a maker <b className="text-danger">above</b> it earns negative CAFC credits (负积分), <b className="text-safe">below</b> it positive (正积分) = (target − actual) × volume. Being above is not non-compliance — it can be cleared with NEV credits. The compliance verdict is the credit balance; see the <button onClick={() => setScreen('creditbook')} className="font-semibold text-brand hover:underline">Credit book</button>.
        </p>
      </Section>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        {(() => {
          // CREDIT scoreboard (积分) — each pool/maker by its dual-credit position,
          // not a "gap to a line". Sort: cost owed · volume · net NEV balance.
          const cnSort = sortByEff === 'units' ? 'units' : sortByEff === 'fine' ? 'cost' : 'credit'
          const cr = (n: number) => `${n >= 0 ? '+' : '−'}${fmtInt(Math.abs(n))}`
          const withCredit = items.map((it) => ({ it, c: creditByScope.get(it.key) ?? creditByScope.get(it.label) ?? { cafc: 0, nev: 0, cost: 0, buy: 0, short: false } }))
          const maxBuy = Math.max(1, ...withCredit.map((x) => x.c.buy))
          withCredit.sort((a, b) => cnSort === 'units' ? b.it.units - a.it.units : cnSort === 'cost' ? b.c.cost - a.c.cost : b.c.buy - a.c.buy)
          return (
            <Section className="rise [animation-delay:420ms]" title={`${sectionLabel} · credit scoreboard`} right={
              <span className="flex items-center gap-0.5 rounded-lg bg-black/[0.04] p-0.5">
                {(['gap', 'units', 'fine'] as const).map((k) => (
                  <button key={k} onClick={() => setSortBy(k)}
                    className={`rounded-md px-2 py-0.5 text-[10px] font-semibold transition ${sortByEff === k ? 'bg-white text-ink-100 shadow-sm' : 'text-ink-500 hover:text-ink-100'}`}>
                    {k === 'gap' ? 'By deficit' : k === 'units' ? 'By volume' : 'By cost'}
                  </button>
                ))}
              </span>
            }>
              <div className="mb-1.5 flex items-center gap-2.5 px-3 text-[9.5px] font-semibold uppercase tracking-wide text-ink-500">
                <span className="w-1.5 shrink-0" /><span className="w-24 shrink-0">Maker</span>
                <span className="flex-1">credits to buy</span>
                <span className="w-16 shrink-0 text-right">CAFC 积分</span><span className="w-16 shrink-0 text-right">NEV 积分</span><span className="w-16 shrink-0 text-right">cost</span>
              </div>
              <div className="space-y-2">
                {withCredit.map(({ it, c }) => (
                  <div key={it.key} onClick={() => it.drillable && drillInto(it.key)}
                    ...
                    <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${c.short ? 'bg-danger' : 'bg-safe'}`} />
                    <span className="w-24 shrink-0 truncate text-sm text-ink-100" title={it.label}>{it.label}</span>
                    <div className="flex-1"><Bar value={c.buy} max={maxBuy} color={c.short ? 'bg-danger' : 'bg-safe'} /></div>
                    <span className={`num w-16 shrink-0 text-right text-[12px] font-semibold ${c.cafc >= 0 ? 'text-safe' : 'text-danger'}`}>{cr(c.cafc)}</span>
                    <span className={`num w-16 shrink-0 text-right text-[12px] font-semibold ${c.nev >= 0 ? 'text-safe' : 'text-danger'}`}>{cr(c.nev)}</span>
                    <span className={`num w-16 shrink-0 text-right text-[11px] font-semibold ${c.cost > 0 ? 'text-danger' : 'text-ink-500'}`}>{c.cost > 0 ? fmtMoney(c.cost, pack.currency) : '—'}</span>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        <Section className="rise [animation-delay:480ms]" title={`Fuel-economy (CAFC) heatmap · ${exploreFocus ? 'models' : 'makers'} × year`} right={<span className="text-[11px] text-ink-500">click → drill</span>}>
