...
export default function Locked({ title, purpose, icon, value, priceLabel, onUnlock }: {
  ...
  return (
    <div className="mx-auto max-w-[720px] py-16 text-center">
      <span className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-[#F6F2EB]/[0.06] text-[#B8AEA0]">
        <Icon name={icon} size={22} />
      </span>
      <h1 className="font-display mt-5 text-[22px] font-bold tracking-[-0.02em] text-[#F6F2EB]">{title}</h1>
      <p className="mx-auto mt-2 max-w-[52ch] text-[13.5px] leading-relaxed text-[#7E756A]">{purpose}</p>

      {value ? (
        <div className="mx-auto mt-8 max-w-[46ch] rounded-2xl border border-white/[0.07] bg-[#17140F] px-7 py-6">
          <div className="text-[10.5px] font-semibold uppercase tracking-[0.13em] text-[#7E756A]">On your own data</div>
          <div className="dnum mt-2.5 text-[30px] font-extrabold tabular-nums tracking-[-0.02em]" style={{ color: STATUS.compliant }}>
            {value.headline}
          </div>
          <p className="mt-2 text-[12.5px] leading-relaxed text-[#7E756A]">{value.detail}</p>
        </div>
      ) : (
        <p className="mx-auto mt-8 max-w-[48ch] text-[12.5px] leading-relaxed text-[#7E756A]">
      ...
      <button onClick={onUnlock}
        className="mt-8 inline-flex items-center gap-2 rounded-lg bg-brand px-5 py-2.5 text-[13px] font-semibold text-[#100E0C] transition hover:brightness-110">
